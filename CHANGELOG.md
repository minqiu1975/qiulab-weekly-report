...
## 2026-05-25: 基准库全面更新 - 替换真实文献 + 研究方向 + 同步机制

### 步骤1: 替换Mock文献为真实数据
- `src/data/mockLiterature.ts`: 完全替换为 PAINT Lab 仇旻实验室真实发表论文
- 新增20篇真实论文（2023-2025年），来源: https://qiu.lab.westlake.edu.cn/ky/fblw.htm
- 包含标志性成果：4H-SiC超透镜、纹身水熊虫冰刻技术、3.8g AR波导等
- `searchLiterature()`: 增加 `source` 字段搜索支持

### 步骤2: 更新推荐关键词
- `src/components/LiteratureSearch.tsx`: 关键词更新为实验室实际研究方向
- 新关键词：SiC超透镜、冰刻技术、AR光波导、拓扑光子学、光计算、钙钛矿、超表面、飞秒激光加工

### 步骤3: 添加研究方向介绍卡片
- `src/pages/LiteraturePage.tsx`: 新增6大核心研究方向展示卡片
  - 碳化硅超表面与AR光波导
  - 冰刻纳米加工技术
  - 拓扑光子学
  - 光计算与智能推断
...
- 添加"检查更新"按钮
- 7天未同步自动控制台提示
- 备注：因浏览器CORS限制，完整自动同步需后端代理支持

---

## 2026-05-25: 修复趋势页面日期错误，添加上传时间时区标识

### 问题1: 趋势页面显示不存在的 2026.02.13 日期
- **原因**: 之前修复期数时错误地将 `2026.02.12` 改为 `2026.02.13`，但用户没有 2月13日的数据
- **修复**: 
  - `src/data/mockTrends.ts`: 恢复 `2026.02.12`，移除 `2026.02.13`
  - `src/App.tsx`: 添加启动清理逻辑，移除用户 localStorage 中残留的 `2026.02.13` 动态标签/趋势/历史数据

### 问题2: 上传时间显示不正确（缺少时区信息）
- **原因**: `formatDate` 只显示本地时间，没有时区标识
- **修复**: `src/pages/UploadPage.tsx`: `formatDate` 添加 UTC 偏移标识（如 `(UTC+08:00)`）
